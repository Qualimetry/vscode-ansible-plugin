...
class NoWorldWritableCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        NoWorldWritableCheck check = new NoWorldWritableCheck();
    ...
    @Test
    void whenModeNotWorldWritable_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Create config
    ...
    @Test
    void whenModeIsWorldWritable_reportsIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Create config
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-restrict-world-write");
    }

    @Test
    void whenTaskNotFileModule_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Ping
