...
class OnlyBuiltinsCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        OnlyBuiltinsCheck check = new OnlyBuiltinsCheck();
    ...
    @Test
    void whenBuiltin_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Copy
    ...
    @Test
    void whenCommunityModule_reportsIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: S3
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-builtin-modules-only");
    }
}
