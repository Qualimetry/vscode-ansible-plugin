...
class RiskyOctalCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        RiskyOctalCheck check = new RiskyOctalCheck();
    ...
    @Test
    void whenNumericMode_reportsNoIssue() {
        // Use decimal to ensure mode is parsed as number, not string octal
        String yaml = """
            - hosts: all
              tasks:
                - name: Copy
    ...
    @Test
    void whenStringOctalMode_reportsIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Copy
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-numeric-file-mode");
    }

    @Test
    void whenNotFileModule_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Ping
