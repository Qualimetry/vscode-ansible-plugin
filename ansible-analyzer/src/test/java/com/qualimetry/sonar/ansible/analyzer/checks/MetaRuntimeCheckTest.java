...
class MetaRuntimeCheckTest {
    ...
    private AnsibleContext runRoleMetaCheck(RoleMeta meta) {
        ...
        MetaRuntimeCheck check = new MetaRuntimeCheck();
    ...
    @Test
    void whenAllowDuplicatesFalse_reportsNoIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of(), List.of(), List.of(), false, URI, null);
        assertThat(runRoleMetaCheck(meta).getIssues()).isEmpty();
    }

    @Test
    void whenAllowDuplicatesTrue_reportsIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of(), List.of(), List.of(), true, URI, null);
        AnsibleContext context = runRoleMetaCheck(meta);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-role-meta-runtime");
    }
}
