...
class LineLengthCheckTest {
    ...
    @Test
    void whenAllLinesShort_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Ping
    ...
    @Test
    void whenOneLineExceeds160_reportsIssueOnThatLine() {
        String longLine = "x".repeat(161);
        String yaml = "- hosts: all\n  tasks:\n    - name: " + longLine;
        AnsibleContext context = runCheck(yaml);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-max-line-length");
