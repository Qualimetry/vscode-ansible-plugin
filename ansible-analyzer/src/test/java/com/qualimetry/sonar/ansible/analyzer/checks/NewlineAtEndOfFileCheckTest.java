...
class NewlineAtEndOfFileCheckTest {
    ...
    @Test
    void whenEndsWithNewline_reportsNoIssue() {
        String yaml = "- hosts: all\n";
        assertThat(runCheck(yaml).getIssues()).isEmpty();
    }

    @Test
    void whenDoesNotEndWithNewline_reportsFileIssue() {
        String yaml = "- hosts: all";
        AnsibleContext context = runCheck(yaml);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-file-ends-newline");
        assertThat(context.getIssues().get(0).line()).isNull();
    }

    @Test
    void whenEmpty_reportsNoIssue() {
        assertThat(runCheck("").getIssues()).isEmpty();
    }
}
