...
class MaxPlaysPerPlaybookCheckTest {
    ...
    private AnsibleContext runCheck(PlaybookFile file) {
        ...
        MaxPlaysPerPlaybookCheck check = new MaxPlaysPerPlaybookCheck();
    ...
    @Test
    void whenPlaybookHasFewPlays_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks: []
            - hosts: db
    ...
    @Test
    void whenPlaybookHasMoreThan10Plays_reportsIssue() {
        List<Play> plays = new ArrayList<>();
        for (int i = 0; i < 11; i++) {
            plays.add(new Play("P" + i, List.of(), List.of(), i + 1, List.of()));
        }
        PlaybookFile file = new PlaybookFile(plays, URI);
        AnsibleContext context = runCheck(file);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-limit-plays");
        assertThat(context.getIssues().get(0).message()).contains("11");
    }
}
