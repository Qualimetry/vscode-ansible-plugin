...
class RoleNameCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        RoleNameCheck check = new RoleNameCheck();
    ...
    @Test
    void whenRoleNameLowercaseUnderscore_reportsNoIssue() {
        String yaml = """
            - hosts: all
              roles:
                - my_role
    ...
    @Test
    void whenRoleNameWithCaps_reportsIssue() {
        String yaml = """
            - hosts: all
              roles:
                - MyRole
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-role-name-format");
    }

    @Test
    void whenRoleNameWithDashes_reportsIssue() {
        String yaml = """
            - hosts: all
              roles:
                - role-with-dashes
    ...
    @Test
    void whenFqcnRole_reportsNoIssue() {
        String yaml = """
            - hosts: all
              roles:
                - role: ansible.builtin.something
            """;
        assertThat(runCheck(yaml).getIssues()).isEmpty();
    }
}
