...
class MetaVideoLinksCheckTest {
    ...
    private AnsibleContext runRoleMetaCheck(RoleMeta meta) {
        ...
        MetaVideoLinksCheck check = new MetaVideoLinksCheck();
    ...
    @Test
    void whenValidVideoLinks_reportsNoIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of(), List.of(), List.of("https://example.com/video"), null, URI, null);
        assertThat(runRoleMetaCheck(meta).getIssues()).isEmpty();
    }

    @Test
    void whenInvalidVideoLink_reportsIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of(), List.of(), List.of("not-a-url"), null, URI, null);
        AnsibleContext context = runRoleMetaCheck(meta);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-role-meta-video-links");
    }
}
