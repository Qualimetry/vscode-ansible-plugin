...
class NoTrailingWhitespaceCheckTest {
    ...
    @Test
    void whenNoTrailingWhitespace_reportsNoIssue() {
        String yaml = "- hosts: all\n  tasks: []\n";
        assertThat(runCheck(yaml).getIssues()).isEmpty();
    }

    @Test
    void whenLineEndsWithSpace_reportsIssueOnThatLine() {
        String yaml = "- hosts: all \n  tasks: []\n";
        AnsibleContext context = runCheck(yaml);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-strip-trailing-whitespace");
        assertThat(context.getIssues().get(0).line()).isEqualTo(1);
    }

    @Test
    void whenEmptyLine_reportsNoIssue() {
        String yaml = "\n\n";
        assertThat(runCheck(yaml).getIssues()).isEmpty();
    }
}
