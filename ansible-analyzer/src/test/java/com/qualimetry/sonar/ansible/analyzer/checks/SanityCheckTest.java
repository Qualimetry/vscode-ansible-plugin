...
class SanityCheckTest {
    ...
    @Test
    void whenPlaybook_reportsOneInformationalIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Ping host
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-runtime-sanity");
        assertThat(context.getIssues().get(0).message()).contains("ansible-test sanity");
    }
}
