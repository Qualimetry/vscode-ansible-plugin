...
class IgnoreErrorsCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        IgnoreErrorsCheck check = new IgnoreErrorsCheck();
    ...
    @Test
    void whenNoIgnoreErrors_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Ping
    ...
    @Test
    void whenIgnoreErrorsYes_reportsIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Maybe fail
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-explicit-error-handling");
    }
}
