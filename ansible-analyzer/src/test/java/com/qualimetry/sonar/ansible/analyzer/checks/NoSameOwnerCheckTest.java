...
class NoSameOwnerCheckTest {
    ...
    private AnsibleContext runCheck(String yaml) {
        ...
        NoSameOwnerCheck check = new NoSameOwnerCheck();
    ...
    @Test
    void whenNoSameOwner_reportsNoIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Copy
    ...
    @Test
    void whenSameOwnerTrue_reportsIssue() {
        String yaml = """
            - hosts: all
              tasks:
                - name: Copy
        ...
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-explicit-owner-group");
    }
}
