...
class GalaxyCheckTest {
    ...
    private AnsibleContext runRoleMetaCheck(RoleMeta meta) {
        ...
        GalaxyCheck check = new GalaxyCheck();
    ...
    @Test
    void whenValidDependencies_reportsNoIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of("some.role", Map.of("role", "other.role")), List.of(), List.of(), null, URI, null);
        assertThat(runRoleMetaCheck(meta).getIssues()).isEmpty();
    }

    @Test
    void whenDependencyMissingRoleOrName_reportsIssue() {
        RoleMeta meta = new RoleMeta(Map.of(), List.of(Map.of("version", "1.0")), List.of(), List.of(), null, URI, null);
        AnsibleContext context = runRoleMetaCheck(meta);
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).ruleKey()).isEqualTo("qa-role-galaxy-deps");
    }
}
