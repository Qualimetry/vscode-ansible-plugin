/**
 * Fetches an Ansible quality profile from SonarQube and converts it to ansibleAnalyzer.rules format.
 */

const ANSIBLE_LANGUAGE = 'ansible';
const REPO_KEY = 'qualimetry-ansible';
...
export async function fetchQualityProfiles(config: SonarConfig): Promise<QualityProfile[]> {
    ...
    const url = `${base}/api/qualityprofiles/search?language=${encodeURIComponent(ANSIBLE_LANGUAGE)}`;
    const baseHeaders: Record<string, string> = {
        'Accept': 'application/json',
        'User-Agent': 'Qualimetry-Ansible-Extension/1.0',
    };
