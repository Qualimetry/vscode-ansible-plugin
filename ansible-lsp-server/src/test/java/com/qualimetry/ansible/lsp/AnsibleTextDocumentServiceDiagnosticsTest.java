...
class AnsibleTextDocumentServiceDiagnosticsTest {
    ...
    @Test
    void toDiagnostic_setsClickableCodeDescription() {
        Issue issue = new Issue("qa-spaces-not-tabs", "Do not use tabs", null, 2, null, null);
        Diagnostic d = AnsibleTextDocumentService.toDiagnostic(issue, LspConfig.defaults());
        assertThat(d.getCodeDescription()).isNotNull();
        assertThat(d.getCodeDescription().getHref())
                .isEqualTo("https://github.com/Qualimetry/sonarqube-ansible-plugin/blob/main/docs/rules/qa-spaces-not-tabs.md");
    }
